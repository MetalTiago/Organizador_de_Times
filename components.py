...
def build_input_container(app_state):
    ...
    def save_user(e):
        ...
        if not selected_list_ids:
             # --- CORRIGIDO (Sintaxe 0.28 - Cores como Strings) ---
             state.page.snack_bar = ft.SnackBar(ft.Text(get_string(app_state, "select_at_least_one_list_error")), bgcolor="red_700"); state.page.snack_bar.open = True; state.update(); return 
             # --- FIM DA CORREÇÃO ---
        try:
...
def build_edit_container(app_state):
    ...
    if app_state.page: app_state.page.overlay.append(file_picker_edit)
