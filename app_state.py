...
import os 
import json 
from localization import get_string # Importa get_string
...
class AppState:
    ...
    def __init__(self, page):
        ...
        # Define a lista ativa inicial com base no valor salvo, padrão 0 se não encontrado ou inválido
        self.active_list_id = self.config.get("last_active_list_id", 0) 
        # Guarda o número de times preferido, padrão 2 se não encontrado ou inválido
        ...
        # --- Componentes da UI ---
        # Define o valor inicial do slider e do texto com base na preferência carregada
        self.team_count_slider = ft.Slider(
            min=2, max=10, divisions=8, 
            value=self.preferred_team_count, # Usa o valor carregado
            label="{value}", width=250
        )
        # Define o texto inicial do contador de times
        self.team_count_text = ft.Text(
            get_string(self, "teams_count", count=self.preferred_team_count), # Usa o valor carregado
            size=16, weight="bold"
        ) 
        ...
        # ... (Resto dos componentes e referências) ...
    ...
    # --- FUNÇÃO ATUALIZADA PARA CARREGAR CONFIGURAÇÕES ---
    def _load_config(self):
        # Define os padrões para todas as chaves
        default_config = {
            "language": "pt_br", 
            "theme_mode": "dark",
        ...
        try:
            if os.path.exists(self.config_file_path):
                with open(self.config_file_path, 'r', encoding='utf-8') as f:
                    ...
                    # Mescla os padrões com o carregado, garantindo que todas as chaves existam
                    config = default_config.copy()
                    config.update(loaded_config) 
                    
                    # Validações (já existentes e novas)
                    if config["language"] not in ["pt_br", "en_us", "es"]:
                        config["language"] = default_config["language"]
                    if config["theme_mode"] not in ["dark", "light"]:
                        config["theme_mode"] = default_config["theme_mode"]
                    if not isinstance(config.get("last_active_list_id"), int): # Garante que é int
                        config["last_active_list_id"] = default_config["last_active_list_id"]
                    if not isinstance(config.get("last_team_count"), int) or not (2 <= config["last_team_count"] <= 10): # Garante int e dentro do range
                         config["last_team_count"] = default_config["last_team_count"]
    ...
    # --- FUNÇÃO ATUALIZADA PARA SALVAR CONFIGURAÇÕES ---
    def save_config(self, key, value):
        """Salva uma chave e valor específicos no arquivo de configuração."""
        try:
    ...
    # --- FUNÇÕES DE ATALHO PARA SALVAR PREFERÊNCIAS ---
    def save_language_preference(self, lang_code):
        self.save_config("language", lang_code)

    def save_theme_preference(self, theme_mode):
        self.save_config("theme_mode", theme_mode)
        
    def save_last_list_preference(self, list_id):
        self.save_config("last_active_list_id", list_id)
        
    def save_last_team_count_preference(self, count):
        self.save_config("last_team_count", count)
