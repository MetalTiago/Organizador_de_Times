...
class AppState:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.team_count_slider = ft.Slider(min=2, max=10, divisions=8, value=self.preferred_team_count, label="{value}", width=250)
    ...
    # --- SOLUÇÃO DEFINITIVA (page.open) ---
    def show_snack_bar(self, message: str, color: str = "green_700", icon: str = "check_circle_outline"):
        """Exibe uma mensagem usando a API moderna page.open()."""
        if not self.page: return
        
        snack_content = ft.Row(
            [
                ft.Icon(name=icon, color="white"),
                ft.Text(message, color="white", expand=True, weight="bold", text_align=ft.TextAlign.LEFT),
            ],
        ...
        snack_bar = ft.SnackBar(
            ...
            bgcolor=color,
            action=get_string(self, "ok_button", default="OK"),
            action_color="white",
            duration=4000 
        )
        
        # A API 0.21+ usa page.open() para garantir que o overlay funcione
        # independentemente de atualizações de view
