...
import os
import base64
from localization import get_string, get_team_color_name # Importa ambas as funções

# Remove a lista fixa de cores daqui
# cores_times = [...]

# Cores fixas (valores hex)
cores_times_hex = [
    "#D32F2F", "#1976D2", "#388E3C", "#FBC02D", "#7B1FA2", 
    "#F57C00", "#0097A7", "#C2185B", "#689F38", "#303F9F",
]

async def share_teams(state, teams_data):
    text_to_copy = get_string(state, "results_title") + ":\n\n"
    for i, team_data in enumerate(teams_data):
        nome_cor = get_team_color_name(state, i) # Usa a nova função para obter o nome da cor
...
def build_results_view(state):
    def update_team_cards_layout():
        ...
        for i, team_data in enumerate(state.resultado_times):
            nome_cor = get_team_color_name(state, i) # Obtém o nome da cor traduzido
            valor_cor_hex = cores_times_hex[i % len(cores_times_hex)] # Pega o valor hex da lista
            ...
            team_cards.append(ft.Container(
                content=ft.Card(elevation=4, margin=0, content=ft.Column([
                    ft.Container(
                        content=ft.Row([
                            ft.Text(team_name_str, weight="bold", size=16, color=ft.colors.WHITE, expand=True, text_align=ft.TextAlign.CENTER), 
                            ft.IconButton(
                                icon=ft.icons.SHARE, 
                                icon_color=ft.colors.WHITE, 
                                icon_size=18, 
                                tooltip=get_string(state, "copy_button_tooltip", team_name=team_name_str), 
                                on_click=lambda _, t=team_data, n=team_name_str: state.page.run_task(share_single_team, state, t, n)
                            )
                        ]), 
                        bgcolor=valor_cor_hex, # Usa o valor hex para a cor de fundo
                    ...
                    ft.Container(
                        content=ft.Column([
                            players_column, 
                            ft.Divider(height=5, color=ft.colors.with_opacity(0.5, ft.colors.ON_SURFACE)), 
                            ft.Row([
                                ft.Text(get_string(state, "total_skill_label"), weight="bold"), # Traduzido
                                ft.Text(f"{sum(j[2] for j in team_data)}", weight="bold")
