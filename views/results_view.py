...
async def share_teams(state, teams_data):
    ...
    # --- CORREÇÃO DE COR ---
    state.page.snack_bar = ft.SnackBar(ft.Text(get_string(state, "all_teams_copied_success")), bgcolor="blue_700"); state.page.snack_bar.open = True; state.page.update() 
    # --- FIM DA CORREÇÃO ---
...
async def share_single_team(state, team_data, team_name):
    ...
    state.page.set_clipboard(text_to_copy)
    # --- CORREÇÃO DE COR ---
    state.page.snack_bar = ft.SnackBar(ft.Text(get_string(state, "team_copied_success", team_name=team_name)), bgcolor="blue_700"); state.page.snack_bar.open = True; state.page.update() 
    # --- FIM DA CORREÇÃO ---
...
def build_results_view(state):
    ...
    def update_team_cards_layout():
        ...
        for i, team_data in enumerate(state.resultado_times):
            ...
            for j in sorted(team_data, key=lambda x: x[1]):
                # --- CORREÇÃO DE COR / ÍCONE ---
                avatar_display = ft.Container(width=28, height=28, content=ft.Icon(name="person_outline"), border_radius=14, bgcolor=apply_opacity("white", 0.1)) 
                # --- FIM DA CORREÇÃO ---
                if len(j) > 3 and j[3]:
    ...
    async def reorganize_teams_click(page: ft.Page, e=None):
        ...
        # --- Lógica de Fase 1.5 (Removendo verificação Pro) ---
        print("Organizando...")
...
# A } extra foi removida. O arquivo termina aqui.