...
async def share_teams(state, teams_data):
    ...
    # --- CORREÇÃO (API SnackBar) ---
    # Revertemos para a sintaxe síncrona que funciona
    state.page.snack_bar = ft.SnackBar(
        ft.Text(get_string(state, "all_teams_copied_success")), 
        bgcolor="blue_700"
    )
...
async def share_single_team(state, team_data, team_name):
    ...
    # --- CORREÇÃO (API SnackBar) ---
    state.page.snack_bar = ft.SnackBar(
        ft.Text(get_string(state, "team_copied_success", team_name=team_name)), 
        bgcolor="blue_700"
    )
    state.page.snack_bar.open = True
    state.page.update() # <--- update() síncrono
...
def build_results_view(state):
    ...
    async def share_all_teams_click(e):
        if state.resultado_times: 
            await share_teams(state, state.resultado_times)
        else: 
            # --- CORREÇÃO (API SnackBar) ---
            state.page.snack_bar = ft.SnackBar(
                ft.Text(get_string(state, "generate_teams_first_error")), 
                bgcolor="red_700"
            )
            state.page.snack_bar.open = True
            state.page.update() # <--- update() síncrono
