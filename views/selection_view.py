...
def build_selection_view(state):
    ...
    # Função para atualizar a contagem de selecionados (usa get_string)
    def update_selected_count():
         state.selected_count_text.value = get_string(state, "selected_players_count", count=len(state.selecionados))

    # Função toggle (sem alterações na lógica de salvar)
    ...
    # Função para montar a lista (sem alterações na lógica de salvar)
    def montar_lista_jogadores():
        ...
        for jogador in users:
            is_selected = any(s[0] == jogador[0] for s in state.selecionados)
            avatar_display = ft.Container(width=44, height=44, content=ft.Icon(ft.icons.PERSON_OUTLINE), border_radius=22, bgcolor=ft.colors.with_opacity(0.1, ft.colors.ON_SURFACE))
        ...
        update_selected_count() # Atualiza a contagem inicial
    state.montar_lista_jogadores = montar_lista_jogadores

    # Função para limpar seleção (sem alterações na lógica de salvar)
    def clear_selection(e):
        state.selecionados.clear()
        montar_lista_jogadores()
        state.update()

    # --- ATUALIZA A FUNÇÃO PARA SALVAR A PREFERÊNCIA DO SLIDER ---
    def update_team_count_text_and_save(e):
        ...
        state.update()
    # Liga a nova função ao evento on_change
    state.team_count_slider.on_change = update_team_count_text_and_save 

    # Constrói a view (já usa os valores iniciais definidos em app_state.py)
    view = ft.Column([
