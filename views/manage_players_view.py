...
def build_manage_players_view(state: 'AppState'):
    ...
    def save_changes(e):
        ...
        try:
            ...
            state.show_snack_bar(get_string(state, "list_updated_success"), "blue_700", "check_circle_outline")
        except Exception as ex:
            state.show_snack_bar(get_string(state, "save_error", error=ex), "red_700", "error_outline")

    active_list_name = get_list_name(state.active_list_id) if state.active_list_id != 0 else ""
    header = ft.Row([ ft.IconButton(icon="arrow_back", on_click=lambda e: state.navigate_to("main"), tooltip=get_string(state, "back_button_tooltip")), ft.Row([ ft.Text(get_string(state, "manage_list_title"), size=16), ft.Text(f'"{active_list_name}"', size=16, weight="bold", color="primary"), ft.IconButton(icon="edit_outlined", on_click=lambda e: state.open_rename_list_dialog(e), tooltip=get_string(state, "rename_list_tooltip"), icon_size=20) ], spacing=5, alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER, ), ft.Container(width=40) ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER)
    view = ft.Column(controls=[header, ft.Divider(), players_checkbox_list, ft.Divider(), ft.Row([ft.FilledButton(get_string(state, "save_changes_button"), icon="save", on_click=save_changes, expand=True, height=50)], alignment=ft.MainAxisAlignment.CENTER)], expand=True)
    return view