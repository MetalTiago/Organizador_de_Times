...
def build_main_view(state: 'AppState'):
    ...
    def update_menu_state():
        ...
        if hasattr(state, 'rename_menu_item') and state.rename_menu_item is not None:
            ...
            # --- CORREÇÃO DO ASSERTION ERROR ---
            # Só chamamos .update() se o componente já estiver anexado à página.
            # Caso contrário (na construção inicial), as propriedades acima 
            # serão aplicadas quando ele for renderizado pela primeira vez.
            if hasattr(state, 'list_management_menu') and state.list_management_menu: 
                if state.list_management_menu.page:
    ...
    # Chama a função para preencher o dropdown INICIALMENTE
    ...
    def open_new_list_dialog(e):
        ...
        dialog = ft.AlertDialog(title=ft.Text(get_string(state, "create_list_dialog_title")), content=new_list_name, actions_alignment=ft.MainAxisAlignment.END, modal=True, shape=ft.RoundedRectangleBorder(radius=10))
        def save_new_list(e):
            ...
            if list_name_value:
                try:
                    ...
                    state.show_snack_bar(f"Lista '{list_name_value}' criada!", "green_700", "check_circle_outline")
                    ...
                    state.page.update()
        dialog.actions = [ ft.TextButton(get_string(state, "save_button"), on_click=save_new_list), ft.TextButton(get_string(state, "cancel_button"), on_click=lambda _: state.page.close(dialog)) ]
    ...
    def open_rename_list_dialog(e):
        ...
        dialog = ft.AlertDialog(title=ft.Text(get_string(state, "rename_list_dialog_title")), content=new_list_name_edit, actions_alignment=ft.MainAxisAlignment.END, modal=True, shape=ft.RoundedRectangleBorder(radius=10))
        def save_new_name(e):
            ...
            if new_name_value and new_name_value != current_name:
                try:
                    ...
                    state.page.close(dialog) 
                    state.navigate_to("main")
                    state.show_snack_bar(get_string(state, "list_renamed_success"), "blue_700", "edit")
                except Exception as ex:
                     ...
                     state.page.update()
        dialog.actions = [ ft.TextButton(get_string(state, "save_button"), on_click=save_new_name), ft.TextButton(get_string(state, "cancel_button"), on_click=lambda e: state.page.close(dialog)) ]
        state.page.open(dialog)
    ...
    def delete_current_list(e):
        ...
        if len(state.lists_dropdown.options) <= 2:
            state.show_snack_bar(get_string(state, "cannot_delete_last_list_error"), "red_700", "warning_amber_rounded")
            return 
        list_name = get_list_name(state.active_list_id)
        dialog = ft.AlertDialog(title=ft.Text(get_string(state, "delete_confirmation_title")), content=ft.Text(get_string(state, "delete_list_confirmation_content", list_name=list_name)), modal=True, shape=ft.RoundedRectangleBorder(radius=10))
        def confirm_delete(e):
            ...
            try:
                ...
                state.save_last_list_preference(0)
                state.navigate_to("main")
                state.show_snack_bar(get_string(state, "list_deleted_success", list_name=list_name), "red_700", "delete_outline")
            except Exception as del_ex: 
                state.show_snack_bar(f"Erro ao excluir: {del_ex}", "red_700", "error_outline")
        dialog.actions = [ ft.TextButton(get_string(state, "yes_button"), on_click=confirm_delete, style=ft.ButtonStyle(color="red")), ft.TextButton(get_string(state, "no_button"), on_click=lambda e: state.page.close(dialog)) ]
        state.page.open(dialog)
