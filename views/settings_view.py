...
def build_settings_view(state):
    ...
    # --- Funções de Importação/Exportação (sem alterações na lógica interna) ---
    def save_file_result(e: ft.FilePickerResultEvent):
        if e.path:
            try:
                ...
                state.page.snack_bar = ft.SnackBar(ft.Text(get_string(state, "export_success")), bgcolor=ft.colors.GREEN_700)
            except Exception as ex:
                state.page.snack_bar = ft.SnackBar(ft.Text(get_string(state, "export_error", error=ex)), bgcolor=ft.colors.RED_700)
    ...
    def perform_import(file_path, mode):
        ...
        try:
            with open(file_path, "r", encoding="utf-8-sig") as csvfile:
                ...
                for row in reader:
                    ...
                    if not list_id:
                        ...
                        if existing_list: list_id = existing_list[0]
                        else: list_id = create_list(list_name); added_lists += 1
            ...
            summary_message = get_string(state, "import_summary",
                                         added_players=added_players, added_lists=added_lists,
                                         updated_players=updated_players, skipped_players=skipped_players)
            state.page.snack_bar = ft.SnackBar(ft.Text(summary_message), bgcolor=ft.colors.GREEN_700, duration=4000)
        except Exception as ex:
            state.page.snack_bar = ft.SnackBar(ft.Text(get_string(state, "import_error", error=ex)), bgcolor=ft.colors.RED_700)
    ...
    def open_file_result(e: ft.FilePickerResultEvent):
        if e.files:
            ...
            import_mode_selection = ft.RadioGroup(
                content=ft.Row([
                    ft.Radio(value="ignore", label=get_string(state, "import_mode_ignore")),
                    ft.Radio(value="overwrite", label=get_string(state, "import_mode_overwrite"))
                ]),
                value="ignore"
            )
            ...
            confirm_dialog = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton(get_string(state, "cancel_button"), on_click=lambda e: setattr(confirm_dialog, 'open', False) or state.update()),
                    ft.ElevatedButton(get_string(state, "continue_import_button"), on_click=continue_import)
                ],
    ...
    view = ft.Column(
        controls=[
            ft.Row([
                ...
                state.theme_toggle_button if state.theme_toggle_button else ft.Container(width=40)
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER),
            ft.Divider(),

            # --- CORREÇÃO APLICADA AQUI ---
            # Envolvemos a Row com um Container para aplicar o padding
            ft.Container(
                content=ft.Row([language_dropdown]),
                padding=ft.padding.symmetric(horizontal=10) # Padding aplicado ao Container
            ),
            # --- FIM DA CORREÇÃO ---

            ft.Divider(),
            ft.ListView(
                expand=True, # Adiciona expand=True para que a ListView preencha o espaço restante
                controls=[
                    ft.ListTile(
